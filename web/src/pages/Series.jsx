...
export default function Series() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Priorité TMDB, fallback Xtream
      const tmdb = await fetchFirstOk([
        `/tmdb/tv/${id}-mapped`,
        `/tmdb/tv/${id}`
      ]);
      const meta = tmdb ?? (await fetchFirstOk([`/xtream/series/${id}`]));

      if (!alive) return;
      if (!meta) { setErr("Aucune information trouvée."); setLoading(false); return; }

      // Uniformisation des champs pour coller à la page film
      const title =
        meta.title || meta.name || meta.original_name || meta.stream_display_name || "Série";
      const overview =
        meta.overview || meta.plot || meta.description || "";
      const poster =
        meta.poster || meta.poster_path || meta.cover || meta.image || "";
      const backdrop =
        meta.backdrop_path || meta.backdrop || meta.fanart || "";
      const year =
        (meta.first_air_date || meta.release_date || meta.year || "").toString().slice(0, 4);

      // Trailer
      let trailerUrl = meta.trailer_url || "";
      if (!trailerUrl && Array.isArray(meta.videos)) {
        const yt = meta.videos.find(
          v => (v.site === "YouTube" || v.host === "YouTube") && ((v.type || v.kind) === "Trailer")
        );
  ...
  return (
    <div className="relative">
          ...
          <div className="flex items-end gap-4">
            {data.poster ? (
              <img
                src={data.poster}
                alt={data.title}
                className="hidden md:block w-32 md:w-36 lg:w-40 rounded-xl shadow-xl object-cover"
                loading="lazy"
              />
            ) : null}
            <div>
              ...
              <p className="mt-3 max-w-3xl text-zinc-200 line-clamp-[8] md:line-clamp-[6]">
                {data.overview || "Aucun résumé."}
              </p>
              <div className="mt-4 flex gap-3">
                {data.trailerUrl ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center rounded-lg bg-white text-black px-4 py-2 text-sm font-medium hover:opacity-90"
                  >
                    Bande-annonce
                  </a>
                ) : (
      ...
      {/* Contenu additionnel si besoin */}
      <div className="px-4 md:px-8 lg:px-12 py-8">
        {/* Placeholder sections comme sur films si tu en as */}
        {/* Ex: casting, saisons, recommandations */}
      </div>
    </div>
  );
}
