...
export default function Movies() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setErr(null);
        const res = await fetch(`${API_BASE}/xtream/movies`, {
          method: "POST",
          credentials: "include",
  ...
  return (
    <section className="mx-auto max-w-6xl">
      <h1 className="mb-4 text-2xl font-semibold">Films</h1>
      ...
      {!loading && !err && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6">
          {items?.map((m) => (
            <article
              key={m.stream_id}
              className="group rounded-xl bg-zinc-900/60 p-2 ring-1 ring-white/10"
              title={m.name}
            >
