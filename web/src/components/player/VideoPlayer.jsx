...
export default function VideoPlayer({
  ...
  useEffect(() => {
    ...
    return () => { destroyed = true; const p = playerRef.current; playerRef.current = null; if (p) p.destroy(); };
  }, [src, initialTime]);
  ...
  return (
    <div className="relative w-full">
      <video ref={videoRef} className="w-full h-auto bg-black rounded-xl" poster={poster} controls playsInline preload="metadata" />
      <div className="absolute right-3 top-3 flex gap-2">
        <select className="rounded bg-black/60 text-white text-xs px-2 py-1"
        ...
        <select className="rounded bg-black/60 text-white text-xs px-2 py-1"
          value={textSel.enabled ? (textSel.lang || "") : ""} onChange={(e) => applyText(e.target.value || null)}>
